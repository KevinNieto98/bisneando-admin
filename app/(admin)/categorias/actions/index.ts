...
export async function putCategoriasAction(
  id: number,
  nombre_categoria: string,
  is_active: boolean,
  icono?: string | null
) {
  // Construimos el payload evitando sobreescribir icono si no viene
  const payload: {
    nombre_categoria: string;
    is_active: boolean;
    icono?: string | null;
  } = { nombre_categoria, is_active };
  ...
  const { data, error } = await supabase
    ...
    .select("id_categoria,nombre_categoria,is_active,icono")
