...
import {
  postCategoriasAction,
  getCategoriasAction,
  putCategoriasAction,
} from "./actions";

// 🔧 Utils (con alias para evitar choque de nombres)
import {
  clampPage,
  ensureIcon,
  filterCategorias,
  nextId,
  slicePage,
  totalPages as getTotalPages, // 👈 alias
} from "./utils";
...
export default function CategoriasPage() {
  ...
  // Filtrado + paginado (usando utils)
  const filtered = useMemo(() => filterCategorias(data, query), [data, query]);
  const pages = useMemo(() => getTotalPages(filtered.length, PAGE_SIZE), [filtered.length]); // 👈 usa alias
  ...
  const handleToggleActiva = async (id: number, next?: boolean) => {
    ...
    try {
      ...
      // Reconciliar con la respuesta de la DB
      setData((prev) =>
        prev.map((c) =>
    ...
    try {
      if (exists) {
        ...
        const created = await postCategoriasAction(
          editing!.nombre_categoria,
          editing!.activa,
          ensureIcon(editing!.icono) // fallback "Tags"
        );
    ...
    <div className="max-w-7xl mx-auto px-6 py-4">
      ...
      {!hasNoResults && (
        <div className="mt-2 flex justify-center">
          <Pagination
            totalPages={pages}     // 👈 usa la const pages
