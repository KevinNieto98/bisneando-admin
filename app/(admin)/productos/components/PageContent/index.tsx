...
/** Map nombre de ícono (string) -> componente compatible con MenuCard.Icon */
const iconFromName = (name?: string | null) => {
  const fallback = "Boxes"; // ícono por defecto para Productos
...
export  function PageContent() {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        // 👇 Cargamos los accesos del menú "PRODUCTOS"
        const data = await getMenusAction("PRODUCTOS");
        setMenus(data);
      } catch (e) {
        console.error(e);
        setError("No se pudieron cargar los accesos de Productos.");
        setMenus([]);
      }
    })();
  }, []);
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 pb-8 space-y-8">
      ...
      {isLoading && (
        <nav
          aria-label="Submenú de Productos (cargando)"
      ...
      {hasNoResults && (
        <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-neutral-300 p-8 text-center">
          ...
          <div className="text-sm text-neutral-600">
            Aún no hay accesos configurados para el menú de Productos.
          </div>
        </div>
      )}
